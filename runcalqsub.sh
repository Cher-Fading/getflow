...
while IFS= read -r line; do
	for ((i = 1; i <= $4; i++)); do
		...
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqloop.sh@" ~/getflow/runcalq_PC$c$i.job
		...
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqloop.sh@" ~/getflow/runcalq_CC$c$i.job
