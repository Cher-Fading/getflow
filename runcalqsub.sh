...
cd ~/getflow
	...
	cp ~/getflow/run_temp.job ~/getflow/runcalq_PC$c$i.job
        sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqsubloop.sh@" ~/getflow/runcalq_PC$c$i.job
...
chmod +777 /pnfs/usatlas.bnl.gov/users/cher97/$1_pnfs_$c'_PC_calq_subdiff'$2_v$3
	...
	cp ~/getflow/run_temp.job ~/getflow/runcalq_CC$c$i.job
        sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqsubloop.sh@" ~/getflow/runcalq_CC$c$i.job
