#!/bin/bash
#./runhadd.sh $1 foldername:spmethod $2 tag $3 data $4:nsubs
...
while IFS= read -r line <&3; do
    ...
    sed -i "s@^Arguments.*@Arguments       = \$(Process) $folder $nsubs@" ~/getflow/condors/$folder'_runhadd.job'
    sed -i "s@^Queue.*@Queue $((nhadd))@" ~/getflow/condors/$folder'_runhadd.job'
