...
while IFS= read -r line; do
	...
	d=${line#*AOD.}
		...
		cp ~/getflow/run_temp.job ~/getflow/runcalqdiffsub_PC$c.job
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqdiffsubloop.sh@" ~/getflow/runcalqdiffsub_PC$c.job
		sed -i "s@^Arguments.*@Arguments       = $1 _pnfs $c PC \$(Process) $2 $3 $4@" ~/getflow/runcalqdiffsub_PC$c.job
		nof=$(($(wc -l <~/getflow/$1_pnfs_$c'_PC_rootlist.txt') - 2))
		sed -i "s@^Queue.*@Queue $nof@" ~/getflow/runcalqdiffsub_PC$c.job
		...
		cp ~/getflow/run_temp.job ~/getflow/runcalqdiffsub_CC$c.job
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqdiffsubloop.sh@" ~/getflow/runcalqdiffsub_CC$c.job
		sed -i "s@^Arguments.*@Arguments       = $1 _pnfs $c CC \$(Process) $2 $3 $4@" ~/getflow/runcalqdiffsub_CC$c.job
		nof=$(($(wc -l <~/getflow/$1_pnfs_$c'_CC_rootlist.txt') - 2))
		sed -i "s@^Queue.*@Queue $nof@" ~/getflow/runcalqdiffsub_CC$c.job
