...
while IFS= read -r line; do
	...
	for ((i = 1; i <= $8; i++)); do
		...
		cp ~/getflow/run_temp.job ~/getflow/runcalqdiffsub_PC$c$i.job
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqdiffsubloop.sh@" ~/getflow/runcalqdiffsub_PC$c$i.job
		sed -i "s@^Arguments.*@Arguments       = $1 _pnfs $c PC \$(Process) $2 $3 $4 $i@" ~/getflow/runcalqdiffsub_PC$c$i.job
		nof=$(($(wc -l <~/getflow/$1_pnfs_$c'_PC_rootlist.txt') - 2))
		sed -i "s@^Queue.*@Queue $nof@" ~/getflow/runcalqdiffsub_PC$c$i.job
		...
		condor_submit runcalqdiffsub_PC$c.job
		...
		cp ~/getflow/run_temp.job ~/getflow/runcalqdiffsub_CC$c$i.job
		sed -i "s@^Executable.*@Executable   = /usatlas/u/cher97/getflow/runcalqdiffsubloop.sh@" ~/getflow/runcalqdiffsub_CC$c$i.job
		sed -i "s@^Arguments.*@Arguments       = $1 _pnfs $c CC \$(Process) $2 $3 $4 $i@" ~/getflow/runcalqdiffsub_CC$c$i.job
		nof=$(($(wc -l <~/getflow/$1_pnfs_$c'_CC_rootlist.txt') - 2))
		sed -i "s@^Queue.*@Queue $nof@" ~/getflow/runcalqdiffsub_CC$c$i.job
		...
		condor_submit runcalqdiffsub_CC$c.job
	done
